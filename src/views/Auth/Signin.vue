<template>
  ...
  ></alert>
  <alert v-if="success" variant="success" title="Thành công" message="Đăng nhập thành công"></alert>
</template>
...
<script setup lang="ts">
  ...
  const handleSubmit = async () => {
    loading.value = true;
    try {
      const res = await login(userName.value, password.value, keepLoggedIn.value);
      if (res.status === 200) {
        success.value = true;
        setTimeout(() => {
          success.value = false;
